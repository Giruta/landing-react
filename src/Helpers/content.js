const content  = {
  'header': {
    'nav': {
      'about': 'О нас',
      'gallery': 'Галерея',
      'program': 'Программа курса',
      'reviews': 'Отзывы',
      'contacts': 'Контакты',
    },
    'inner': {
      'title': 'Мандала',
      'subtitle': 'творческая встреча',
      'innerText': 'За 30 дней вы поймаете дзен, почувствуете счастье в моменте и создадите шедевр.',
      'btn': 'Записаться',
    }
  },
  'services': {
    'title': 'Узнаете себя?',
    'list': {
      'first': 'Рутина поглотила всю жизнь?',
      'second': 'Давно не отключалась от проблем?',
      'third': 'Мечтали рисовать, но решили, что это сложно и нет таланта?',
      'fourth': 'Хочу здесь и сейчас, художественная школа это долго и некогда.?',
    }
  },
  'statistic': {
    'first': {
      'number': '7893',
      'span': 'человека',
      'info': 'По всему миру прошли обучение',
    },
    'second': {
      'number': '5',
      'span': 'лет',
      'info': 'Курс существует c 2014 года',
    },
    'third': {
      'number': '15',
      'span': 'минут',
      'info': 'В день и вы достили Дзен',
    },
    'fourth': {
      'number': '30',
      'span': 'дней',
      'info': 'Займет весь период обучения',
    },
    'btn': 'Записаться'
  },
  'leader': {
    'title_begin': 'Меня зовут',
    'titleName': 'Марина Мельник',
    'title_end': 'и я',
    'list': {
      'first': '11 лет управляет программами в международной IT-компании',
      'second': 'Получила международные сертификации Professional Scrum Master I, Certified Scrum Professional и Certified Scrum Master',
      'third': 'Эксперт в управлении процессами, качеством, поставками персоналом, рисками и взаимоотношениями с клиентами в IT-аутсорсинге',
      'fourth': 'Когда-то тоже думала, что не умею рисовать',
      'fifth': 'Прошла долгий путь от всем недовольного человека до эксперта в психологии счастья',
      'sixth': 'Создаю и веду курсы по саморазвитию',
      'seventh': 'Совершенно уверена, что в каждом из нас живет творец! И надо лишь помочь ему раскрыться',
    }
  },
  'thinking': {
    'title': 'Вы все еще думаете, что наш Урок не для Вас?',
    'list': {
      'first': {
        'first': 'Невозможно нарисовать неправильно. Важен процесс, а не результат',
        'second': 'Думаете, что творчество - это для детей, и в 30/40/50 лет уже поздно и друзья засмеют...Но все равно ' +
                  'втайне мечтаете о своем первом шедевре!',
        'third': 'Не знаете как быстро восстановить энергию и порадовать внутреннюю девочку',
      },
      'second': {
        'first': 'Рисование мандалы - легальный способ ничего не делать',
        'second': 'Вы хотите сразу прекрасный результат, и у вас нет терпения и возможности учиться годами',
        'third': 'Вам мешают творить страхи: не получится/не умею/руки не от туда растут',
      },
    },
    'btn': 'Записаться'
  },
  'sliderTitle': 'Они не умели рисовать, но у них все получилось! Присоединяйся!',
  'goals': {
    'title': 'Это больше, чем просто рисование',
    'subtitle': 'Эти милые завитушки могут кое-что изменить в вашей жизни',
    'list': {
      'first': {
        'title': 'Средство от хандры и стресса',
        'info': 'Суета мегаполиса, "день сурка", хроническая усталость знакомы многим. Рисование мандалы поможет отключить внутренний монолог, снять напряжение и приятно разнообразить будни',
      },
      'second': {
        'title': 'Развитие правого полушария мозга',
        'info': 'Правое полушарие отвечает за интуицию, воображение, способность мечтать и даже заниматься сексом. Мандала дает ему нагрузку и развивает наши творческие способности.',
      },
      'third': {
        'title': 'Стимул начать творить',
        'info': 'Творчество делает нашу жизнь яркой, гармоничной и вдохновенной.\n' +
          '                      Техника рисования мандалы проста и доступна каждому, это позволит\n' +
          '                      впустить творчество в свою жизнь и сделать ее более счастливой',
      },
      'fourth': {
        'title': 'Победа над внутренним критиком',
        'info': 'В процессе обучения мы снимем такие психологические барьеры к творчеству,\n' +
          '                      как излишний перфекционизм, самокритика, зависимость от мнения окружающих',
      },
    },
    'btn': 'goals_button',
  },
  'timetable': {
    'title': 'timetable_title_content',
    'list': {
      'first': {
        'title': 'first_title_content',
        'info': 'first_info_content',
      },
      'second': {
        'title': 'second_title_content',
        'info': 'second_info_content',
      },
      'third': {
        'title': 'third_title_content',
        'info': 'third_info_content',
      },
      'fourth': {
        'title': 'fourth_title_content',
        'info': 'fourth_info_content',
      },
      'fifth': {
        'title': 'fifth_title_content',
        'info': 'fifth_info_content',
      },
      'sixth': {
        'title': 'sixth_title_content',
        'info': 'sixth_info_content',
      },
      'seventh': {
        'title': 'seventh_title_content',
        'info': 'seventh_info_content',
      },
      'eighth': {
        'title': 'eighth_title_content',
        'info': 'eighth_info_content',
      },
  },
    'btn': 'timetable_button',
  },
  'reviewsTitle': 'reviews_title_content',
  'signup': {
    'title': 'signup_title_content',
    'form': {
      'title': 'form_title_content',
      'input': {
        'meet': 'input_meet_content',
        'meetText': 'input_meet_text_content',
        'email': 'input_email_content',
        'name': 'input_name_content',
        'phone': 'input_phone_content',
      },
      'text': {
        'begin': 'begin_text',
        'privacy': 'privacy_text',
        'and': 'and_text',
        'offer': 'offer_text'
      },
    },
    'btn': 'signup_button',
  },
  'present': {
    'title': 'present_title_content',
    'btn': 'present_button',
  },
  'contact': {
    'title': 'contact_title_content',
    'address': 'address_content',
    'text_first': 'contact_text_first',
    'text_second': 'contact_text_second',
  },
  'footer': 'footer_text',
}

export default content;
